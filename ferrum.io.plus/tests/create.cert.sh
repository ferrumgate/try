...
EOF
    ...
    cd certs
    cp server.cert.pem ../docker_ssl/nginxconfig/ssl/example.com.crt
    cp server.key.pem ../docker_ssl/nginxconfig/ssl/example.com.key

    if [ $(ls ferrumgate* 2>/dev/null | wc -l) -eq 0 ]; then
        mkcert ferrumgate.com "www.ferrumgate.com" localhost "127.0.0.1" "192.168.88.10" "::ffff:192.168.88.10" "::1"
    fi
fi
